...
// 0. GET: Fetch ALL Announcements (including archived)
router.get('/announcements/all', authenticateToken, async (req, res) => {
    try {
        const queryText = 'SELECT * FROM Announcements ORDER BY date_posted DESC;';
...
// 0. GET: Fetch ALL Events
router.get('/events/all', authenticateToken, async (req, res) => {
    try {
        const queryText = 'SELECT * FROM Events ORDER BY event_date DESC;';
        const result = await db.query(queryText);
        res.status(200).json(result.rows);
    } catch (error) {
...
// 0. GET: Fetch CURRENT Timetable
router.get('/timetables/current', authenticateToken, async (req, res) => {
    try {
        const queryText = "SELECT * FROM Timetables WHERE status = 'current' ORDER BY date_posted DESC LIMIT 1;";
        const result = await db.query(queryText);
        if (result.rowCount === 0) {
            return res.status(404).json({ message: "No current timetable found." });
        }
